...
により実現することができる．1個目の操作は，座標軸を$z$軸を基準に時計回りに$π/4$回転させることで実現されるが，一般に$θ$回転は行列$T$を用いて，
$$
\begin{equation*}
    V
    =
...
\end{equation*}
$$
で与えられるから，上記の回転に対する変換行列$T^{(1)}$は，
$$
\begin{equation*}
    V
    =
    \begin{pmatrix}
        \frac{1}{\sqrt{2}} & \frac{1}{\sqrt{2}} & 0 \cr
        -\frac{1}{\sqrt{2}} &  \frac{1}{\sqrt{2}} & 0 \cr
           0   &    0    & 1
    \end{pmatrix}
\end{equation*}
$$
である．2個目の操作は，$x$軸を基準に時計回りに回転させることで実現される．
